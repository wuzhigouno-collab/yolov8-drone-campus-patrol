...
| 260924 0820 | E0 重跑起跑成功（agent-1，新会话 id）：实例 pull 至 c33919c（工作区干净无 stash），tmux 会话 e0 内训练推进至 epoch1 iter152/203（约 2-4 it/s），ZeroDivisionError/interpolate 计数 0，multi_scale=0.5 修正实证生效；实际 batch=32 未触发降级，OOM 以 TaskAlignedAssigner CPU 回退形式非致命发生、loss 正常下降；显存 23497/24564 MiB 平稳。save_dir=runs/e0_baseline2（旧崩溃 run 的 e0_baseline 残留致 name 自增，未触碰）；快照按脚本设计在训练退出后落盘。教训：实例 tmux 启动训练须 new-session+send-keys（非交互 shell 无 conda PATH 秒退） |
| 260924 0910 | E0 崩溃（agent-1 例行检查发现）：epoch 10 iter ~91/203 硬 OOM（bbox_iou 分配失败，23.49G 占满），退出码 1；此前 TaskAlignedAssigner CPU 回退警告为前兆。**脚本 OOM 自动降级未触发**（与 2215 条"自动降 16"矛盾，疑降级逻辑缺陷或触发条件不符）。崩溃前峰值 epoch8 mAP50 0.311/mAP50-95 0.182；results.csv 10 行、best/last.pt、config_snapshot.yaml 已落盘（崩溃现场完整保留未动）。实验矩阵暂停，派根因调查：①降级逻辑为何未触发 ②batch=16+multi_scale=0.5 显存是否够 |
| 260924 0925 | 根因查明（agent-2）：脚本本无自动降级，昨日"自动降 16"实为 ultralytics 内置 OOM 重试（trainer.py:444-463），**仅覆盖首 epoch、最多 3 次**；今日崩溃在 epoch 10 越过该边界直接抛——非脚本缺陷。正确动作即脚本注释设计路径：BATCH=16 人工减半重跑（nbs=64 自动 accumulate=4，等效 batch 不变，口径无碍）。安全性夹逼：昨日 batch=16+True(上限1920) 峰值 22.7G 未 OOM，今日 batch=32+0.5(上限1440) OOM；batch=16+0.5 峰值估 13-16G，余量≥7G，安全。裁决：崩溃残留 e0_baseline2 保留不删（可追溯），BATCH=16 重跑，新 run 预计 e0_baseline3；内置降级边界与 send-keys 两条教训并入轻量批处理待沉淀 |
| 260924 0940 | E0 BATCH=16 重跑起跑（agent-1）：save_dir=runs/e0_baseline3，epoch5 iter83/405（8-10 it/s，比 batch=32 快近倍），全程零 OOM 警告（对照：上次崩溃 run 从起跑就持续 CPU 回退警告）。显存 epoch4 峰值 22.8G 超预估区间（13-16G）但系分配器缓存高水位+multi_scale 尺寸抖动的高位振荡，非单调泄漏；裁决：不干预继续跑——峰值挺过即缓存复用，再 OOM 则重启加 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True。监控 cron 已更新指向新 run |
| 260924 1005 | 轻量批处理完成（agent-3 生产/agent-0 复核 PASS）：README_TRAIN.md:74 改 0.5+指引、run_exp.sh 注释行号区间修正 120-135（interpolate 实在 :135，验证员亲读源码实证）、lessons 新增两条（内置 OOM 重试仅首 epoch 边界、tmux 须 send-keys）；零代码改动，提交推送 |
| 260924 1100 | 用户决策：实验矩阵走串行训练（覆盖 1831 条 BATCH_SIZE=2 两两并行），E1a–E1d 逐个派出，实例单任务运行 |
| 260924 1105 | 用户修正：仍走并行训练（恢复 1831 条 BATCH_SIZE=2 口径，1100 条串行决策作废）；E1a–E1d 两两并行，单轮 batch 届时按双任务显存预算下调（accumulate 保持等效 batch） |
| 260924 1150 | **E0 完训**（09:15-11:03，1h49m，退出码 0，100 轮无早停）：best epoch59 mAP50 0.4303/mAP50-95 0.2597，末轮 epoch100 0.4344/0.2629（文献区间 0.33-0.40 上方，符合预期）；runs/e0_baseline3 三件套中 results.csv/weights/快照/预测样张齐，**但实验登记未自动写入**（register 未触发，原因待查——R3 验收时手动调用 exit=0，疑设计为手动子命令或自动挂接缺陷）。派 agent-1 收尾：查明 register 触发方式并执行登记+rsync 回传，再派验证抽查 |
| 260924 1210 | E0 收尾完成（agent-1）：register 查明为手动子命令设计（run_exp.sh:44/766/920，训练路径不自动调，非缺陷）；登记已写入（E0 行，mAP50 0.43578/mAP50-95 0.26452）。**勘误 1150 条**：最优实为 mAP50 0.43578 @ epoch 89（agent-1 自纠 awk 错列，epoch59/0.4303 作废）；mAP50-95 登记值 0.26452 与 epoch89 行 0.26417 存 0.00035 差（疑脚本各列独立取最优，验证抽查中裁决口径）。回传 scp 至 archive/exp_backup/260924/ 逐文件核对一致（results.csv 101 行、权重字节数、样张、日志全对）；教训补记：非交互 ssh 调脚本需 bash -lc（与 tmux 同类 PATH 坑） |
| 260924 1225 | **E0 ✅ 验收 PASS**（agent-0）：三件套齐、登记数字逐字溯源 results.csv（mAP50 列最大值 0.43578@ep89、mAP50-95 列最大值 0.26452@ep88——cmd_register 为各列独立取最优逻辑）、配置摘要 14 项与快照一致、负结果备注如实。**口径裁决**：CSV 登记维持现状（可溯源即合规）；论文表格统一改"best.pt 所在 epoch 行同行取值"（E0 即 ep89 行 0.43578/0.26417），差 0.00035 不影响结论，写论文素材时执行。E1a 并行对（degrees=10/30，BATCH=8×2）派出 |
| 260924 1245 | E1a 并行对起跑（agent-4）：e1a10（runs/e1a_deg10，batch=8）与 e1a30（runs/e1a_deg30，batch=8→iter0 OOM 被内置重试降 batch=4）双组稳定推进，合计显存 19.3G<23G。**裁决：保留 e1a30 batch=4 继续**——nbs=64 accumulate 下等效 batch 两组同 64，受试变量为 degrees，微 batch 差异快照/日志如实留痕，杀掉重跑浪費机时。调用口径修正：E1a 两组为独立子命令 e1a10/e1a30（degrees 内部固定、run 名天然区分）；实测双 batch=8 并行会 OOM，后续并行对单任务 batch 按 8+4 或更低预算。register 为手动子命令，各实验完训后须显式执行 |
| 260924 1650 | E1a-d10 完训收尾（agent-4）：退出码 0 满 100 轮，峰值 mAP50 0.42535@ep86 / mAP50-95 0.25336@ep90，**对照 E0 基线 0.43578 为 -1.04pt 负结果**（如实登记保留）；登记+回传 archive/exp_backup/260924/e1a_deg10/ 逐文件核对一致。e1a30 在训 epoch 73/100（峰值 0.38634@ep71），独享 GPU 提速至 19 it/s；趁势并行派 E1b（batch=8），e1a30 完训后同流程收尾 |
| 260924 1730 | E1a-d30 完训（14:19 退出码 0，early stop 77 轮，峰值 0.38634@ep71）——对照 E0 为 -4.9pt 负结果；E1b 首次起跑（batch=8）与 e1a30 合计 23.8G 触线，裁决杀 E1b 降 BATCH=4 重启（runs/e1b_cutout2，旧 run 留痕），重启后 e1a30 已自然完训 GPU 全空——裁决 E1b 维持 batch=4 不再折腾（等效 batch 64 口径无碍）。E1a-d10 验证 PASS（agent-0）✅。派 e1a30 收尾+E1c 并行起跑 |
| 260924 1800 | E1a-d30 验证 PASS（agent-0）✅：三件套齐、登记值与 results.csv 逐字一致（0.38932@ep76 / 0.1964@ep57）、early stop@ep77 自洽、-4.6pt 负结果如实。观察留痕：①快照 batch 字段记 8 系框架行为（args.yaml 启动时落盘、内置降级不回写），实际 4 以备注+日志为准；②**勘误 1730 条**：e1a30 峰值应为 0.38932@ep76（1730 条沿用中途读数 0.38634@ep71，作废），论文素材一律以 results.csv 为准。E1c 已起跑（runs/e1c_invert，batch=8，与 E1b 并行合计 18.5G 安全）；**E1a 全部完成：d10 -1.04pt、d30 -4.6pt，旋转增强对俯视场景均为负增益** |
| 260924 1755 | E1c 完训收尾（agent-4）：满 100 轮退出码 0，峰值 mAP50 0.42062@ep90 / mAP50-95 0.25357@ep89，**对照 E0 为 -1.5pt 负结果**（反转色如预期无增益，如实登记）；回传核对一致。e1b 在训 epoch 81/100（峰值 0.42845@ep80 仍爬升），独享 GPU 提速；并行派 E1d（copy_paste=0.1，BATCH=8）+ E1c 验证抽查 |
| 260924 1815 | E1c 验证 PASS（agent-0）✅：三件套齐、登记值逐字溯源、快照含 albu=InvertImg(p=0.2)、-1.5pt 负结果如实。E1d 起跑（agent-4）：runs/e1d_copypaste，batch=8 copy_paste=0.1（cmd_e1d 经 COPY_PASTE 注入，与 E0 同口径单项差异），与 e1b 并行合计 20.5G 安全；监控 cron 已切 E1b+E1d 对 |
| 260924 1855 | E1b 完训收尾（agent-4）：满 100 轮退出码 0，峰值 mAP50 0.42935@ep86 / mAP50-95 0.26093@ep86，**对照 E0 为 -0.64pt**（已完训单项增强中最接近基线者）；备注如实含首跑被杀留痕说明；回传核对一致。e1d 在训 epoch 43/100（峰值 0.4111@ep42 爬升中），独享 GPU；派 E1b 验证抽查 |
| 260924 1910 | E1b 验证 PASS（agent-0）✅：三件套齐、两列最优值同处 ep86 行逐字一致（本轮无跨行问题）、快照 batch=4 即实际值（无降级不回写差异）、-0.64pt 负结果与插曲链如实。E1 消融已完 5/6 项，仅剩 e1d 在训 |
| 260924 1930 | E1d 完训收尾+验证 PASS（agent-4/agent-0）✅：满 100 轮，峰值 0.43276/0.26306 同@ep83，-0.3pt 轻微负增益如实。**E1 单项消融全部完成（6/6 验证 PASS）**：全部单项增强均未超 E0 基线——copy_paste -0.3pt≈持平、CoarseDropout -0.64pt、旋转10° -1.04pt、反转色 -1.5pt、旋转30° -4.6pt。实验矩阵暂停，实例 GPU 已空；E1e 组合与 E2–E6 排程报用户裁决 |
| 260924 2000 | **用户决策：E1e 与 E2–E6 全部暂停，实例关机**。关机前回传完整性核对完成（agent-4）：6 组成功实验+5 类崩溃/bug 留痕+6 份训练日志全部回传 archive/exp_backup/260924/ 逐文件核对一致，实例 runs//logs/ 全清单无遗漏；数据盘 2.5G/50G。实例待用户在 AutoDL 控制台关机（关机保留数据盘，非释放）。恢复时待裁决项：E1e 组合（copy_paste+CoarseDropout 或跳过）、E2 yolov8s、E3–E6 测量类 |
